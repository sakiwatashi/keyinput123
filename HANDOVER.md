...
- 最新版本：**v0.8.1**，安裝檔由 CI 自動建置發布（推 tag 才會產生）
...
## 20. 0.8.1（2026-09-12）：選字寫法的兩類修正

0.8.0 發布後重新審查，用詞庫自己的高頻詞打了 1000 個詞，從不符的結果裡挑出
兩類問題。兩類都是「語料的寫法」而不是排序參數，所以修在資料層。
...
那不是「兩個一樣常用」的統計，是簡轉繁把同一個詞的兩種寫法各收一次，於是誰排
第一只看插入順序——等於沒有依據。權重 >= 5000 的同分組有 41 組。

判準用台灣詞頻表這份獨立來源，而且只在「剛好一個被收錄」時才動。9 組因此拆開；
兩個都收（提升／提昇、了解／瞭解、計劃／計畫、宣佈／宣布）或都沒收（詞匯／詞彙）
表示這份資料分不出來，那就別猜——27 組維持原樣，這一點也寫成了測試。

**一個字被收在它沒有的讀音底下。** 那 的讀音是 ㄋㄚˋ，哪 才是 ㄋㄚˇ。書寫時
很多人用「那」代替「哪」，語料照單全收：

    ㄋㄚˇ            哪 33103  輸給  那 129343
    ㄋㄚˇ ㄍㄜ˙      哪個 46216  輸給  那個 118070

57 組。「那X」在 ㄋㄚˋ 底下完全正常，所以讓出 ㄋㄚˇ 不會讓任何字打不出來，
只是各自回到自己的讀音；讓位不是刪除，往下捲一格還是找得到。

這兩類分開寫在 `tools/build_taiwan_preferred.py` 的兩節裡：前者是「同音兩種寫法
選哪個」，後者是「讀音歸屬」，判準不同，不該混在一起。
