...
- 最新版本：**v0.8.0**，安裝檔由 CI 自動建置發布（推 tag 才會產生）
...
## 19. 0.8.0（2026-09-10）：打字正確性與工程基礎

0.7.1 之後累積 28 個提交。使用者實際回報、追到根因並修掉的：

- **「六扇門」變成「六三們」**：模糊修正拿 libchewing 在被問一個猜出來的讀音時
  不管聲調照樣回答的結果當證據（ㄙㄢˋ ㄇㄣˊ 回「三門」，真正讀音是 ㄙㄢˉ），
  再用只看字的詞頻表放行。模糊路徑改成只認詞庫。
- **連著打兩個空韻聲母掉字**：打「私事」得到「是」、「知識」得到「十」。
  ㄙ 和 ㄕ 都填聲母欄，而規則把「同一欄位、不同符號」當成改錯字。音節裡只有
  一個成分時它本身就是一個字，那時應該開新音節。同欄位相撞的 180 種組合全部
  打過一遍當測試。
- **按住注音不放狂送字**、**Del 刪掉的字又跑回來**、**組字區裡的符號讓整段
  文字消失**、**「一」被下一個字吃掉**（下一步／一樣／因為）。
- **選字品質**：簡轉繁異體字降權（喫→吃、裏→裡、爲→為）、破音字詞頻按讀音
  分開、台灣寫法優先（介面／帳戶／麻痺，52 組）、「一」「不」的本調與變調
  兩種唸法都查得到詞（1524 筆）。

**讀音相近自動改字改成預設關閉。** 這個專案要的自動修正是錯別字
（common_typos.json，等長無歧義的精確替換），不是拿 ㄕ／ㄙ 去猜另一個詞——
後者會改掉本來就打對的字。功能留著，控制台「狀態」分頁可以打開。

新功能：跨機同步個人詞庫、鍵位自訂、你自己的詞頻表（記在詞上不是字上）、
個人選字帶上下文、`tools/missing_words.py` 找出詞庫沒收而你在用的詞。

工程面兩件值得記的：

- **CI 開始跑按鍵行為測試。** 那兩支需要 32 位元 Python 才載得動 libchewing，
  之前只在本機跑——改完轉接層忘了本機跑，CI 會全綠而輸入法是壞的。
  `setup-python` 給得了 x86，PIME 只需要兩個沒有 import 的檔，
  `tools/fetch_pime_runtime.ps1` 從釘住的 commit 抓下來並校驗 SHA256。
- **兩支測試其實什麼都沒驗。** 按鈕煙霧測試用 `PerformClick`，而它會先問
  `CanSelect`，父鏈沒 Show 過就靜靜地什麼也不做——41 顆按鈕裡有 26 顆從來沒
  被按過，兩顆真的壞掉的按鈕因此活了很久。控制字元掃描則沒有涵蓋
  `control_panel/`。兩個都補了。
