...
TextService::TextService(ImeModule* module):
	...
	updateFont_(false),
	// Smart Priority: two vertical columns (1-5 / 6-0). Python also sends
	// candPerRow=2; keep 2 as the default if customizeUI is late.
	candPerRow_(2),
...
void TextService::createCandidateWindow(Ime::EditSession* session) {
	if (!candidateWindow_) {
		...
		if (elementMgr) {
			// Always draw our Japanese two-column window. Modern apps often set
			// pbShow=FALSE to host the list themselves, which hides our surface
			// and drops the vertical-first layout. Keep TSF bookkeeping, but
			// refuse the host takeover.
			BOOL pbShow = TRUE;
...
void TextService::updateCandidates(Ime::EditSession* session) {
	...
	// Keep the custom surface visible even when TSF UI element sinks try to
	// take ownership of drawing.
	candidateWindow_->Show(TRUE);
