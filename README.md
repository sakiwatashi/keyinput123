...
- 會參考 libchewing 詞庫、Rime Essay 轉製的 113,738 條臺灣正體高頻詞，以及使用者確認過的 2–12 字詞，改善「優化、樹葉、人工智慧」一類上下文選字。文字型高頻詞必須再通過整段注音驗證，避免把罕見破音字拼成讀音不符的詞，例如不會把 `ㄅㄟˋ ㄑㄩㄝˋ` 誤判成「貝殼」。
...
原廠簽章版，執行 `.\install.ps1 -DisableUnsignedNativeUi`。檔案正被 TSF 使用時，切換
