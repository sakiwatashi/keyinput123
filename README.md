...
- 編輯選字時會優先列出完整句修正及游標附近的 2–12 字個人／常用詞彙，再列高頻單字；不顯示未經常用詞庫驗證的中途長句猜測。選擇後整個詞一次更新，同時保留逐字編輯。
...
- 組字時會持續以精確注音與離線常用詞庫更新整段第一候選，讓「優仙→優先」「部合理→不合理」不必等到開候選窗才修正。按 `Enter` 時再啟用保守的近音判斷，例如 `ㄧㄥ ㄍㄞ` 的「英該」或把 `ㄣ／ㄥ` 混淆成 `ㄧㄣ ㄍㄞ` 的「音該」，都能判斷為「應該」，不必逐一列舉錯字表面形式。另有 46 條高可信用字規則處理「以經→已經」「迫不急待→迫不及待」等錯字；當次親自選定或個人詞彙鎖定的字不會被覆蓋，按空白則維持原文。
- 自動組字與候選視窗共用同一份詞彙排序；若候選系統已把完整句修正排在第一，組字區會先直接採用，不必按 `↓` 才取得正確答案。按 `↓`、空白、Enter、標點、Shift 或切換輸入框前都會再次同步第一候選；方向鍵只用來改選其他答案。
...
- 常用 `Shift` 標點包括 `？`、`：`、`＋`、`——`、`，`、`。`、`『』` 與 `＂`。
- 單獨注音按空白時會先依字典補上一聲：字典第一候選是中文字就直接採用，例如 `ㄧ→一`、`ㄚ→啊／阿`，而 `ㄙ`、`ㄓ` 也會進入司／思、之／知等中文字候選；第一候選仍是原注音時則開啟候選單，不讓尾端生僻字冒充常用字。原始注音仍保留在第一頁前四項。完整的 `ㄢˊ` 等讀音同樣保留原注音候選。沒有正在組字時，直接按大千聲調鍵 `3`、`6`、`4`、`7` 會輸出 `ˇ`、`ˊ`、`ˋ`、`˙`；`Shift+Space` 輸出 `ˉ`。
...
內建詞庫的順序決定。
完整句的自動組字與候選欄必須呼叫同一個排序來源；不可讓候選第一項優於尚未送出的組字內容。所有送出路徑都會先同步該預設，再視 Enter 是否需要啟用保守近音修正。
