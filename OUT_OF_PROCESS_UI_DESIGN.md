...
| 找不到信標時沿用上次錨點 | 完成 |
| 滑鼠選字（合成選字鍵） | 已實作，端到端待實測 |
| 遊戲政策（前景為防作弊遊戲時不作為） | 完成，含冒煙測試 |
| 真實輸入情境（直向排列） | 使用者已確認 |

### 遊戲政策

輔助行程比對前景程式,命中已知防作弊遊戲、或**連程式名稱都查不到**（受保護
行程的特徵）時即停止服務。停止服務不只是不繪製,而是**關閉管道** —— 因為信標
模式會縮小原廠候選框,而 Python 端是以「寫入是否成功」判斷是否維持縮小。一個
收下更新卻拒絕顯示的輔助行程,會讓被封鎖的應用程式兩邊都沒有候選字。關閉管道
使寫入失敗,Python 立即恢復完整候選,由原廠簽章視窗服務該應用程式。

### 滑鼠選字

點擊轉換成對應的選字數字並以 `SendInput` 送出,完全重用既有的選字路徑,因此
滑鼠與鍵盤不可能行為分歧,也不需要在刻意單向的管道上開回傳通道。視窗為
`WS_EX_NOACTIVATE`,焦點留在應用程式,按鍵循 TSF 正常抵達。此路徑只在遊戲政策
允許時執行,不會對遊戲合成任何輸入。

**驗證現況**：組成部分皆已個別驗證 —— 命中測試、`SendInput` 確實抵達前景應用
程式（合成的數字鍵被輸入法處理成聲調符號）、以及輸入法既有的數字選字。但
「點擊後確實選中候選」的整體流程尚未自動化驗證：以合成按鍵驅動真實注音輸入法
不穩定,`s`/`u` 未能進入組字,無法穩定造出候選頁。這是測試工具的限制,不是產品
缺陷,實測請以人工點擊確認。
...
預設啟用。`%APPDATA%\PinnedBopomofo\candidate-ui.json` 明確寫入
`{"enabled": false}` 才關閉；檔案不存在、缺少欄位或讀取失敗一律視為預設（啟用）。
這個預設之所以安全，是因為失敗方向是退回而非前進：輔助程式缺席或啟動失敗時，
原廠簽章候選框自動接手，打字不受影響。關閉時鏡像完全惰性：不建立執行緒、
