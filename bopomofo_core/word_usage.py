...
# 每寫入這麼多次才存檔一次。save_json_object 會 fsync 加原子替換，而成本隨檔案
# 大小成長——實測 1000 個讀音串（54 KB）一次 3.5 毫秒，5000 個（272 KB）12 毫秒，
# 到上限 20000 個（1.1 MB）是 40 毫秒。每次送出都寫，等於在每一句話後面加上那個
# 數字；送出本身才 3 毫秒。按鍵追蹤就是這樣犯錯的：它每個按鍵都寫，長到 742 KB
# 還拖慢打字。
#
# 代價是行程被硬殺時最多丟 19 筆。onDeactivate 會補寫，所以切走輸入法、換視窗都
# 不會掉；真正會掉的是控制台按「儲存並重啟 PIME」那種外部終止。掉幾筆只是讓某個
# 詞晚一點被學起來（門檻是三次），不會壞掉任何東西——比每句話卡 40 毫秒划算。
FLUSH_EVERY = 20
